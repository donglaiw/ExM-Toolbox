...
from . import corrpts, utils


class SimpleICPOptimization:
    """
    Class for setting up and solve least squares
    optimization
    """
    ...
    def __transform_point_cloud(
        ...
        H = utils.homo_t_matrix(R, t)

        Xe = np.column_stack((x, y, z))
        Xh = utils.euler_to_homo(Xe)
        Xh_transformed = np.transpose(H @ Xh.T)
        Xe_transformed = utils.homo_to_euler(Xh_transformed)
