...
def roiSlice(z_init: int, n_slices: int, im_channel: np.ndarray, min_idx=5, h_val: int, thresh_val: int, denoise: bool, mask: bool) -> np.ndarray:
    '''
    ...
    for i in tqdm(range(n_slices)):
        ...
        if denoise:
            ...
            (T, threshInv) = cv.threshold(denoised, thresh=thresh_val, maxval=max_val, type=cv.THRESH_BINARY_INV)
