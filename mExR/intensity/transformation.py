...
def computeTform(cfg: CfgNode):
    ...
    if len(m_transform_type) == 1:
        ...
        param_map['NumberOfSamplesForExactGradient'] = cfg.INTENSITY.NumberOfSamplesForExactGradient
        param_map['MaximumNumberOfIterations'] = cfg.INTENSITY.MaximumNumberOfIterations
        param_map['MaximumNumberOfSamplingAttempts'] = cfg.INTENSITY.MaximumNumberOfSamplingAttempts
        param_map['FinalBSplineInterpolationOrder'] = cfg.INTENSITY.FinalBSplineInterpolationOrder
    ...
    sitk.WriteParameterFile(param_map, os.path.join('./results/transforms/', param_file))


def warpAll(cfg: CfgNode, volMove: str, outputName: str, outDir: str, tformPath: str, resolution: list): 
    '''
    ...
    for channel in tqdm(range(vol_move.shape[1])):
        ...
        imsave(f'{outDir}{outputName}_ch0{channel+1}_warped.tif', result.astype('uint16')) # save as a 16-bit image