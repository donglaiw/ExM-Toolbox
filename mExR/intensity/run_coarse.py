...
def saveMask(cfg: CfgNode):
    ...
    masked_vol = roiSlice(
        z_init=0,
        n_slices=move_vol_anchor.shape[0],
        im_channel=move_vol_anchor,
        min_idx=cfg.FILTER.MASK_INDEX,
        h_val=cfg.FILTER.FILTER_STRENGTH,
        thresh_val=cfg.FILTER.THRESH_LOWER,
        denoise=cfg.FILTER.DENOISE,
        mask=cfg.FILTER.MASK,
    )
    print(f"Done!\n")
