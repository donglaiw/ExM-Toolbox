...
def filterImage(img: np.ndarray, minIdx: int) -> np.ndarray:
    ...
    indices, area = np.unique(
        labeled_img, return_counts=True
    )  # get area and index of objects
    ...
    for i in tqdm(range(n_slices)):
        ...
        if denoise:
            ...
            denoised = cv.fastNlMeansDenoising(
                im_slice.astype("uint8"),
                h=h_val,
                templateWindowSize=7,
                searchWindowSize=21,
            )
            (T, threshInv) = cv.threshold(
                denoised, thresh=thresh_val, maxval=max_val, type=cv.THRESH_BINARY_INV
            )
